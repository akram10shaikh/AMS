...
def staff_login(request):
    if request.method == 'POST':
        ...
        if user is not None and user.role =='Staff':
            login(request, user)
            return redirect('staff_dashboard_org')  # Redirect to dashboard on successful login
        else:
...
# New views
from accounts.forms import UserFormOrg, StaffRegistrationFormOrg
...
def staff_register_org(request):
    if request.method == "POST":
        ...
        staff_form = StaffRegistrationFormOrg()
...
def staff_list_org(request):
    user_organization = getattr(request.user, 'organization', None)
    if user_organization:
        staff_members = Staff.objects.filter(organization=user_organization)
    else:
        # If user has no organization, show empty queryset or handle accordingly
        staff_members = Staff.objects.none()

    organizations = Organization.objects.filter(id=user_organization.id) if user_organization else Organization.objects.none()

    return render(request, 'organization/staff_list_org.html', {'staff_members': staff_members, 'organizations': organizations})


@login_required
def staff_dashboard_org(request):
    return render(request, 'organization/staff_dashboard_org.html')